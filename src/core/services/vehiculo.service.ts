...
import { VehiculoModel } from "../../core/models/vehiculo.model";
import { VehiculoEntity } from "../../database/entities/vehiculo.entity";
import { getRepository } from "typeorm";

export const find = async (params: HttpGetParams) => {
    const { page, limit, ...filters } = params;

    const repository = getRepository(VehiculoEntity);
...
export const create = async (model: VehiculoModel) => {
    const repository = getRepository(VehiculoEntity);

    const exists = await repository.findOne({ matricula: model.matricula });

    if (exists) {
        throw new Error('Exite un vehiculo con la matricula proporcionada.')
    }
...
export const update = async (id: number, model: VehiculoModel) => {
    const repository = getRepository(VehiculoEntity);
